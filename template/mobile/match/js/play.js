...
play.ParseMsg = function(d) {
	...
	if(d.match("bestmove ")){
		...
		for (i=0;i<e.length && i<10;i++) {	
			...
			tmpStr += "<tr style=\"height:40px;\"><td>"+ p +"</td><td>"+ o[2] +"</td><td>"+ o[1] +"</td><td><input type=\"Button\" onclick='play.onmdownchessdblist(\""+i+"\")' value=\"Go!\"></td></tr>";
		}	
		...
		if (depth > 1) {
			...
			bill.cleanLine();
			for (j = 0; (j + pvseek) < e.length && j<2; j++) {
				i = j + pvseek + 1;
				if (e[i]) {
					a = e[i].split("");					
			...
			tmpStr = "<tr><td>" + (depth / 32).toFixed(2) + "</td><td>" + score + "</td><td>" + tmpStr + "</td>"  + "</tr>";
